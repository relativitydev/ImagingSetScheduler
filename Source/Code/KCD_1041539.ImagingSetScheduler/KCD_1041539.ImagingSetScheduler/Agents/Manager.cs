...
using KCD_1041539.ImagingSetScheduler.Repository;
	...
	class Manager : kCura.Agent.AgentBase
	{
		...
		private void ProcessWorkspace(DataRow workspaceRow, IServicesMgr svcMgr, ExecutionIdentity identity, IDBContext eddsDbContext, IServicesProxyFactory servicesProxyFactory)
		{
			...
			try
			{
				...
				IImagingSetSchedulerRepository imagingSetSchedulerRepository= new ImagingSetSchedulerRepository(servicesProxyFactory);
				
				List<RelativityObject> list = imagingSetSchedulerRepository.RetrieveAllImagingSetSchedulesNotWaitingAsync(workspaceArtifactId).ConfigureAwait(false).GetAwaiter().GetResult();
